...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(String token: tokens){
			...
			if(isWord(token)){
				numWords++;
				//counts syllables in each token
				if(token.length() == 1 || token.length() == 2){
					...
					for(int i = 0; i < token.length(); i++){
						...
						if(i == token.length() - 1){
							if(syllables == 0 && token.charAt(token.length()-1) == 'e'){
								syllables++;
							}
						}else{
							char d = Character.toLowerCase(token.charAt(i+1));
							if(
				...
				numSyllables = numSyllables + syllables;
			}else{
				numSentences++;
			}
		}
