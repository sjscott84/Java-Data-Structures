...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size() < numSuggestions && visited.size() < THRESHOLD){
			String toCheck = queue.remove(0);
			if(!visited.contains(toCheck)){
				visited.add(toCheck);
				List<String> checkList = new ArrayList<String>();
				insertions(toCheck, checkList, true);
				substitution(toCheck, checkList, true);
				deletions(toCheck, checkList, true);
				//System.out.println(checkList);
				for(int i = 0; i < checkList.size(); i++){
					if(retList.size() < numSuggestions){
						String returnedWord = checkList.get(i);
						queue.add(returnedWord);
						retList.add(returnedWord);
					}
				}
			}
		}
