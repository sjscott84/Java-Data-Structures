...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		root = new WPTreeNode(first, null);
		queue.add(root);
		
		while(!queue.isEmpty() && visited.size() < 5000){
			WPTreeNode check = queue.remove(0);
			visited.add(check.getWord());
			List<String> checkList = new ArrayList<String>();
			nw.insertions(check.getWord(), checkList, true);
			nw.substitution(check.getWord(), checkList, true);
			nw.deletions(check.getWord(), checkList, true);
			for(String n: checkList){
				if(!visited.contains(n)){
					//System.out.println(n);
					WPTreeNode node = check.addChild(n);
					queue.add(node);
					if(n.equals(last)){
